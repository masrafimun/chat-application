...
const Friends = () => {
    ...
    return (
          ...
          <div className='px-4 pt-[60px] sm:pt-0  my-8'>
            <p className=' text-3xl mb-4'>Friends : </p>

            {
            friendsList.length != 0 ? 
              (friendsList.map((user,index) => 
                    <div key={index} className='flex items-center bg-blue-200 justify-between border mb-6 '>
                        <div className='mr-3 bg-amber-400'>
                            <img className='h-[120px] w-[105px]  border-r' src={user.profilePhoto || profile_image} alt="" />
                        </div>
                        <div className='flex flex-col gap-1 pr-2 sm:pr-20'>
                            <p>Name : {user.name} </p>
                            ...
                            <div className=' flex'>
                               ...
                               <p   className={`px-4 ${isOnline(user._id) ? 'bg-green-500' : 'bg-gray-400'} text rounded-md cursor-pointer mr-4`}>{isOnline(user._id) ? 'Active' : 'Offline'}</p>
                               <Link to={`/message/${user._id}`}  className='px-4 bg-[#24bab8] text rounded-md cursor-pointer mr-4'>Messsage</Link>
                            </div>
                        </div>
